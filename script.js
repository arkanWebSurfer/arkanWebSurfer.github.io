const ServerUrl = "192.168.0.105";
const serverPassword = "arkan";
...
parsedData.forEach((data, i) => {
  ...
  cell6.innerHTML = `<button onclick='removebot("${data.growID}")' type="button">Remove</button>`;
  row.appendChild(cell6);

  const cell7 = document.createElement("td");
  cell7.innerHTML = `<button onclick='warpbot("${data.growID}")' type="button">Warp</button>`;
...
requestAPI("runScript", {
  script:'getBot("' + botName +'"):remove()'
})
}


function warpbot(botName) {
